...
int main() {
    ...
    for (int i = 0; i < size; i++) {
        ...
        printf("%f \t", *ptr); // Указатель вместо Xn[i]
        if (*ptr > 0) { // Указатель вместо Xn[i]
            k++;
            if (k == 2) { s_pos = *ptr; } // Указатель вместо Xn[i]
            else if (k > 2) { break; }
        }
        if (fabsf(*ptr - roundf(*ptr)) < 0.0001f) { n++; } // Указатель вместо Xn[i]
    ...
    if (n == 0) { printf("\nThere are no integer elements"); }
        ...
        printf("\nInteger elements: \t");
        for (int i = 0; i < size; i++) {
            if (fabsf(*ptr - roundf(*ptr)) < 0.0001f) { printf("%f \t", *ptr); } // Указатель вместо Xn[i]
            ptr++; // Увеличение указателя на 1
        }
    }
    ptr = Yk; //
